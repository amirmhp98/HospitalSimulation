...
def increase_floor(the_dict, key):
...
class VisualAnalyzerService:
    ...
    def process_matrix(self):
        for patient in self.simulation_matrix:
            increase_floor(self.total_time_freq_normal, patient['time_spent_in_system'])
            increase_floor(self.waiting_time_freq_normal, patient['time_waited_in_queue'])
            increase_floor(self.service_time_freq_normal,
                                patient['time_spent_in_system'] - patient['time_waited_in_queue'])
            if (patient['type'] == 'normal'):
                increase_floor(self.total_time_freq, patient['time_spent_in_system'])
                increase_floor(self.waiting_time_freq, patient['time_waited_in_queue'])
                increase_floor(self.service_time_freq,
                                    patient['time_spent_in_system'] - patient['time_waited_in_queue'])
            else:
                increase_floor(self.total_time_freq_corona, patient['time_spent_in_system'])
                increase_floor(self.waiting_time_freq_corona, patient['time_waited_in_queue'])
                increase_floor(self.service_time_freq_corona,
                                    patient['time_spent_in_system'] - patient['time_waited_in_queue'])
